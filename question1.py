...
def q1_features(df):
    ...
    # --------------------------------------------------------
    # 3. Rising time (paper Eq. 6)
    # --------------------------------------------------------
    # Rising time is defined as the delay of the strongest tap
    # (the tap with maximum received power) minus the delay of
    # the first (earliest) tap in the snapshot.

    strongest_idx = np.argmax(P, axis=1)

    tau_strongest = tau_array[
        np.arange(len(df)),
        strongest_idx,
    ]

    tau_first = tau_array.min(axis=1)

    rising_time = tau_strongest - tau_first
